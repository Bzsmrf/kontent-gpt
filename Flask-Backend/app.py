...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "https://kontent-gpt4.onrender.com"}})
...
def submit_data():
    try:
        ...
        recorded_text = data.get('recordedText', '')  # Extract 'recordedText' from the data
        
        # Call the function to analyze the language of the recorded text
        language_analysis = input_script_language(recorded_text)
        
        # Extract the language name from the analysis result
        language_name = language_analysis.text
        
        # Based on the detected language, execute the appropriate functionality
        improved_script = improve_script(recorded_text, "Long Form")  # Example type, replace with your actual choice
        
        return jsonify({"success": True, "improvedScript": improved_script})
...
@app.route("/translate_output_language")
def translate_output_language():
    script = request.args.get('script', '')  # Get the script from the query parameters
    input_language = request.args.get('inputLanguage', '')  # Get the input language from the query parameters
    
    # Execute the translation functionality
    prompt = f"""
    Translate the below input script into {input_language}. Script is delimited by triple backticks 
    {script}
    """
    output_script = api_call(prompt=prompt)
    
    # Extract text from the response object
    translated_script = output_script.text
    
    return jsonify({"success": True, "translatedScript": translated_script})
...
@app.route("/submit_with_type", methods=['POST'])
def improve_script():
    script = request.json.get("prompt")
...
# @app.route("/translate_output_language1")
...
# @app.route("/save_feedback_to_csv")
