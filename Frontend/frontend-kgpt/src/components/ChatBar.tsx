...
const ChatBar: React.FC<ChatBarProps> = ({ email }) => {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (requestValue == 2) {
				const response = await axios.post('https://kontent-gpt.onrender.com/submit_with_type', { prompt: recordedText || '', type: "Long Form" });
				...
				const response = await axios.post('https://kontent-gpt.onrender.com/submit_with_type', { prompt: recordedText || '', type: "Short Form" });
				console.log('Response from backend:', response.data)
				let outputString = typeof response.data.output === 'string' ? response.data.output : JSON.stringify(response.data.output);
